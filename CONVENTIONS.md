...
Chat polish lives mainly in `baithak.css`. Empty/loading/error: `ui-states.js` + `.cp-empty`. Mehfil: `mehfil.css`. Brand mark: `public/brand/chaupaal-mark.png` + `public/icon.png` (PWA sizes: `icon-192` / `icon-512` / `icon-maskable-512` / `apple-touch-icon`).

Do not invent a second palette per screen. If a new surface needs a value, add a token first.

## 14. Brand mark & Gathered light

- Use `.cp-mark` / `chaupaal-mark.png` for auth, sidebar, favicon chrome — no emoji/placeholder logos. (Splash uses `splash.png`, a separate full-bleed scene.)
