...
if(auth){auth.onAuthStateChanged(async user=>{if(user){currentUser=user;if(db){const s=await db.collection('users').doc(user.uid).get();if(s.exists){userProfile=s.data();if(typeof AuthProfiles!=='undefined'&&AuthProfiles.hydrateActiveProfile){try{await AuthProfiles.hydrateActiveProfile(user.uid,userProfile);}catch(e){}}if(userProfile.lang){currentLang=userProfile.lang;document.getElementById('langSelect').value=currentLang;}if(typeof hydrateIcebreakersFromUserDoc==='function')hydrateIcebreakersFromUserDoc(userProfile);if(typeof hydratePromptsFromUserDoc==='function')hydratePromptsFromUserDoc(userProfile);if(typeof hydrateProfileTypeFromUserDoc==='function')hydrateProfileTypeFromUserDoc(userProfile);if(userProfile.profileMedia||userProfile.profile?.profileMedia){const media=userProfile.profileMedia||userProfile.profile.profileMedia;if(typeof digitalProfile!=='undefined')digitalProfile.profileMedia=media;try{localStorage.setItem('chaupaal_profile_media',JSON.stringify(media));}catch(e){}}}if(userProfile.profile&&typeof digitalProfile!=='undefined'){['bio','interests','prompts','gender','dateOfBirth','currentCity','occupation'].forEach(k=>{if(userProfile.profile[k]!=null)digitalProfile[k]=userProfile.profile[k];});try{localStorage.setItem('chaupaal_digital_profile',JSON.stringify(digitalProfile));}catch(e){}}}}updateProfileBtn();}});}
