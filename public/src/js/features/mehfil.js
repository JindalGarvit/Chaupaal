...
(function () {
  ...
  /** M7 capacity — keep in sync with server-lib/mehfil-access.js */
  const MAX_PUBLISHERS = 10;
  const MAX_PARTICIPANTS = 40;
  const MAX_CAM_ON = 4;
  const MAX_REMOTE_VIDEO_SUBS = 4;
  const MAX_REMOTE_VIDEO_WITH_MEDIA = 2;
  const VOLUME_PAINT_MS = 200;
  const PRESENCE_PAINT_MS = 350;
  const VIDEO_RECONCILE_MS = 400;
  ...
  const CHAT_SEND_MIN_MS = 650;
  const CHAT_SEND_MAX_PER_MIN = 20;
  const REACT_SEND_MIN_MS = 400;
  const REACT_SEND_MAX_BURST = 8;
  const REACT_BURST_WINDOW_MS = 10000;
  const PRESENCE_COALESCE_MS = 4000;
  let lastChatSendAt = 0;
  let chatSendTimestamps = [];
  let lastReactSendAt = 0;
  let reactSendTimestamps = [];
  let presenceJoinBuffer = [];
  let presenceLeaveBuffer = [];
  let presenceCoalesceTimer = null;
  let lastMediaMomentKey = '';
  let chatSeenKeys = new Set();
  let reactSeenKeys = new Set();
  ...
  let lastKnownFreshUids = new Set();
  let lastKnownFreshNames = new Map();
  ...
  /** M7 scale state */
  const remoteUsers = new Map(); // uid → { user, hasVideo, hasAudio }
  const videoSubUids = new Set();
  let pinnedUid = null;
  let pendingVolumes = null;
  let volumePaintTimer = null;
  let lastVolumePaintAt = 0;
  let presencePaintTimer = null;
  let presencePaintPending = null;
  let videoReconcileTimer = null;
  let lastJoinedAsListenerToast = false;
  let videoPausedNoticeAt = 0;
  ...
  function scheduleChatIdleCollapse() {
    /* keep strip open while typing; no auto-collapse spam */
  }

  function linkifyChatHtml(text) {
    ...
    while ((m = re.exec(raw))) {
      ...
      parts.push(
        `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" data-nav-ignore="1">${esc(url)}</a>${esc(trail)}`
      );
      last = m.index + m[1].length;
    }
    if (last < raw.length) parts.push(esc(raw.slice(last)));
    return parts.join('') || esc(raw);
  }
  ...
  function canPostRoomSocial() {
    if (!currentUser?.uid || !activeChatId) return false;
    if (myRoomRole === 'removed') return false;
    return true;
  }

  function chatFloodBlocked() {
    const now = Date.now();
    if (now - lastChatSendAt < CHAT_SEND_MIN_MS) return true;
    chatSendTimestamps = chatSendTimestamps.filter((t) => now - t < 60000);
    return chatSendTimestamps.length >= CHAT_SEND_MAX_PER_MIN;
  }

  function reactFloodBlocked() {
    const now = Date.now();
    if (now - lastReactSendAt < REACT_SEND_MIN_MS) return true;
    reactSendTimestamps = reactSendTimestamps.filter((t) => now - t < REACT_BURST_WINDOW_MS);
    return reactSendTimestamps.length >= REACT_SEND_MAX_BURST;
  }

  function appendChatLine(v, opts) {
    const o = opts || {};
    const strip = chatStripEl();
    const msgsEl = strip?.querySelector('[data-mehfil-chat-msgs]');
    const preview = strip?.querySelector('[data-mehfil-chat-preview]');
    if (!msgsEl) return;
    const key = o.key || `${v.by || 'sys'}_${v.at || 0}_${String(v.text || '').slice(0, 40)}`;
    if (chatSeenKeys.has(key)) return;
    chatSeenKeys.add(key);
    if (chatSeenKeys.size > 200) {
      chatSeenKeys = new Set([...chatSeenKeys].slice(-120));
    }

    const isSystem = !!(v.system || o.system);
    const row = document.createElement('div');
    row.className =
      'mehfil-chat-line' +
      (v.by === currentUser?.uid ? ' is-me' : '') +
      (isSystem ? ' is-system' : '');
    const name = isSystem
      ? ''
      : v.name && v.name !== v.by
        ? v.name
        : displayNameForUid(v.by);
    const initial = esc((name || '?').slice(0, 1));
    const time = formatChatTime(v.at);
    const textHtml = isSystem ? esc(v.text || '') : linkifyChatHtml(v.text);
    row.innerHTML = isSystem
      ? `<span class="mehfil-chat-system">${textHtml}</span>${time ? `<span class="mehfil-chat-time">${esc(time)}</span>` : ''}`
      : `<span class="mehfil-chat-av" aria-hidden="true">${initial}</span>
         <div class="mehfil-chat-body">
           <div class="mehfil-chat-meta"><span class="mehfil-chat-name">${esc(name)}</span>${
             time ? `<span class="mehfil-chat-time">${esc(time)}</span>` : ''
           }</div>
           <span class="mehfil-chat-text">${textHtml}</span>
         </div>`;
    msgsEl.appendChild(row);
    while (msgsEl.children.length > 80) msgsEl.firstChild.remove();
    if (strip?.classList.contains('is-expanded') || o.forceScroll) {
    ...
    } else if (preview && isSystem) {
      preview.textContent = String(v.text || '').slice(0, 120);
    }
    if (strip?.classList.contains('is-collapsed') && !o.silentUnread) {
      chatUnread += 1;
      const badge = strip.querySelector('[data-mehfil-chat-badge]');
      if (badge) {
        badge.hidden = false;
  ...
  function appendSystemMoment(text) {
    if (!text) return;
    appendChatLine({ text, system: true, at: Date.now(), by: null }, { system: true, silentUnread: true });
  }

  function flushPresenceCoalesce() {
    presenceCoalesceTimer = null;
    const joins = presenceJoinBuffer.splice(0);
    const leaves = presenceLeaveBuffer.splice(0);
    const fmt = (arr) => {
      const names = [...new Set(arr.map((p) => (p.name || 'Someone').split(' ')[0]))];
    ...
    if (joins.length) {
      const label = fmt(joins);
      const line =
        joins.length === 1
          ? tt('mehfil_someone_joined', '{{name}} joined', { name: label })
          : tt('mehfil_others_joined', '{{who}} joined', { who: label });
      appendSystemMoment(line);
      setMehfilStatus(line, 'live');
    ...
    if (leaves.length) {
      const label = fmt(leaves);
      const line =
        leaves.length === 1
          ? tt('mehfil_someone_left', '{{name}} left', { name: label })
          : tt('mehfil_others_left', '{{who}} left', { who: label });
      appendSystemMoment(line);
    }
  }

  function queuePresenceEvent(kind, person) {
    if (!person?.uid || person.uid === currentUser?.uid) return;
    if (kind === 'join') presenceJoinBuffer.push(person);
    else presenceLeaveBuffer.push(person);
    clearTimeout(presenceCoalesceTimer);
    presenceCoalesceTimer = setTimeout(flushPresenceCoalesce, PRESENCE_COALESCE_MS);
  }

  async function sendRoomChat(text) {
    const body = String(text || '').trim().slice(0, 500);
    if (!body || !activeChatId) return false;
    if (!canPostRoomSocial()) {
      if (typeof showToast === 'function') showToast(tt('mehfil_chat_blocked', 'You can’t post in this Mehfil right now'));
      return false;
    }
    if (chatFloodBlocked()) {
      if (typeof showToast === 'function') showToast(tt('mehfil_chat_slow', 'Slow down a bit'));
      return false;
    }
    ...
    try {
      ...
      const pushed = await ref.push(payload);
      lastChatSendAt = Date.now();
      chatSendTimestamps.push(lastChatSendAt);
      appendChatLine(payload, { key: pushed?.key || `local_${payload.at}`, forceScroll: true });
      return true;
    } catch (err) {
  ...
  async function sendRoomReaction(emoji, isSticker) {
    if (!emoji || !activeChatId) return;
    if (!canPostRoomSocial()) {
      if (typeof showToast === 'function') showToast(tt('mehfil_chat_blocked', 'You can’t post in this Mehfil right now'));
      return;
    }
    if (reactFloodBlocked()) return;
    const label = (userProfile?.name || 'You').split(' ')[0];
    if (isSticker) showStickerBurst(emoji, { label });
    else showReactionBurst(emoji, { label });
    ...
    try {
      ...
      lastReactSendAt = Date.now();
      reactSendTimestamps.push(lastReactSendAt);
      if (pushed?.key) reactSeenKeys.add(pushed.key);
    } catch (e) {
  ...
  function openInviteSheet() {
    ...
    sheet.className = 'mehfil-ring-pick mehfil-invite-sheet';
    sheet.setAttribute('role', 'dialog');
    sheet.innerHTML = `
      <div class="mehfil-ring-pick-card">
        <h3>${esc(tt('mehfil_invite_title', 'Invite to Mehfil'))}</h3>
        <p class="mehfil-mod-role">${esc(tt('mehfil_invite_sub', 'Bring friends into this room'))}</p>
        <div class="mehfil-ring-pick-list">
          <button type="button" class="mehfil-ring-pick-row" data-inv-bubble>${esc(tt('mehfil_nudge', 'Invite to chat'))}</button>
          <button type="button" class="mehfil-ring-pick-row" data-inv-link>${esc(tt('mehfil_copy_link', 'Copy link'))}</button>
          <button type="button" class="mehfil-ring-pick-row" data-inv-ring>${esc(tt('mehfil_ring', 'Ring'))}</button>
        </div>
        <div class="mehfil-ring-pick-actions">
          <button type="button" data-inv-cancel>${esc(tt('cancel', 'Cancel'))}</button>
        </div>
      </div>`;
    let handle;
    ...
    if (typeof openLayer === 'function') {
      handle = openLayer(sheet, onDismiss, {
        host: shellHost(),
        role: 'dialog',
        label: tt('mehfil_invite_title', 'Invite to Mehfil'),
      });
    } else {
    ...
    sheet.querySelector('[data-inv-cancel]')?.addEventListener('click', close);
    sheet.querySelector('[data-inv-bubble]')?.addEventListener('click', async () => {
      close();
      await nudgeOthers();
    });
    sheet.querySelector('[data-inv-link]')?.addEventListener('click', async () => {
      close();
      await copyInviteLink();
    });
    sheet.querySelector('[data-inv-ring]')?.addEventListener('click', () => {
      close();
      startMehfilRing(activeChat);
    });
  }
  ...
  async function cleanupEmptyRoomMeta(chatId) {
    ...
    try {
      ...
      if (!base) return;
      // Room chat + reactions are live-only — clear when the room empties (honest retention).
      await Promise.all([
        ...
        rtdbRef(`mehfil/${chatId}/chat`)?.remove(),
        rtdbRef(`mehfil/${chatId}/reactions`)?.remove(),
      ]);
    } catch (e) {}
  }
  ...
  function countLocalPublishers(excludeUid) {
    const skip = excludeUid ? String(excludeUid) : '';
    let n = 0;
    rolesByUid.forEach((role, uid) => {
      if (skip && uid === skip) return;
      if (role !== 'listener') n += 1;
    });
    if (roomHostUid && roomHostUid !== skip && !rolesByUid.has(String(roomHostUid))) n += 1;
    return n;
  }

  function videoSubBudget() {
    if (hasYoutubeOnStage()) return MAX_REMOTE_VIDEO_WITH_MEDIA;
    if (lastKnownFreshCount >= 8) return Math.min(3, MAX_REMOTE_VIDEO_SUBS);
    return MAX_REMOTE_VIDEO_SUBS;
  }

  function scheduleVideoReconcile() {
    clearTimeout(videoReconcileTimer);
    videoReconcileTimer = setTimeout(() => {
      videoReconcileTimer = null;
      reconcileVideoSubscriptions().catch(() => {});
    }, VIDEO_RECONCILE_MS);
  }

  function markTileVideoPaused(tile, uid) {
    if (!tile) return;
    tile.classList.add('mehfil-tile--voice', 'is-video-paused');
    ...
    ph.type = 'button';
    ph.className = 'mehfil-tile-placeholder mehfil-tile-video-paused';
    ph.dataset.uid = String(uid);
    ph.textContent = tt('mehfil_video_paused', 'Video paused to save data — tap to view');
    tile.appendChild(ph);
  }

  async function playRemoteVideoOnTile(uid, user) {
    const tile = ensureRemoteTile(uid, remoteTileLabel({ uid }));
    if (!tile || !user?.videoTrack) return;
    tile.classList.remove('mehfil-tile--voice', 'is-video-paused');
    tile.querySelector('.mehfil-tile-placeholder')?.remove();
    tile.querySelector('.mehfil-tile-avatar')?.remove();
    tile.querySelector('.mehfil-tile-video-paused')?.remove();
    try {
      user.videoTrack.play(tile);
    } catch (e) {}
  }

  async function reconcileVideoSubscriptions() {
    if (!client || !overlayEl) return;
    const budget = videoSubBudget();
    const ranked = [];
    if (pinnedUid && remoteUsers.get(String(pinnedUid))?.hasVideo) {
      ranked.push(String(pinnedUid));
    }
    speakingSticky.forEach((speaking, uid) => {
      if (!speaking) return;
      const id = String(uid);
      if (id === String(localUid || '')) return;
      if (remoteUsers.get(id)?.hasVideo && !ranked.includes(id)) ranked.push(id);
    });
    remoteUsers.forEach((info, uid) => {
      if (info?.hasVideo && !ranked.includes(uid)) ranked.push(uid);
    });
    const keep = new Set(ranked.slice(0, Math.max(0, budget)));

    for (const [uid, info] of remoteUsers) {
      if (!info?.hasVideo || !info.user) continue;
      const tile = overlayEl.querySelector(`.mehfil-tile[data-uid="${uid}"]`);
      if (keep.has(uid)) {
        if (!videoSubUids.has(uid)) {
          try {
            await client.subscribe(info.user, 'video');
            videoSubUids.add(uid);
            await playRemoteVideoOnTile(uid, info.user);
          } catch (e) {
            console.warn('[mehfil] video sub', e);
          }
        }
      } else if (videoSubUids.has(uid) || tile?.querySelector('video')) {
        try {
          await client.unsubscribe(info.user, 'video');
        } catch (e) {}
        videoSubUids.delete(uid);
        markTileVideoPaused(tile, uid);
        if (Date.now() - videoPausedNoticeAt > 20000) {
          videoPausedNoticeAt = Date.now();
          setMehfilStatus(tt('mehfil_video_saving', 'Showing fewer videos to stay smooth'), 'warn');
        }
      }
    }
  ...
  function applyAdaptiveStreamQuality() {
    if (!client) return;
    const n = lastKnownFreshCount;
    const preferLow = n >= 6 || hasYoutubeOnStage();
    try {
      if (typeof client.setRemoteDefaultVideoStreamType === 'function') {
        client.setRemoteDefaultVideoStreamType(preferLow ? 1 : 0);
      }
    } catch (e) {}
    remoteUsers.forEach((info, uid) => {
      if (!info?.user) return;
      const hi = String(uid) === String(pinnedUid) || speakingSticky.get(uid) === true;
      try {
        if (typeof client.setRemoteVideoStreamType === 'function') {
          client.setRemoteVideoStreamType(info.user.uid, hi && !preferLow ? 0 : 1);
        }
      } catch (e) {}
    });
  }

  function syncEffectsScale() {
    try {
      if (typeof MehfilEffects?.setParticipantScale === 'function') {
        MehfilEffects.setParticipantScale(lastKnownFreshCount);
      }
    } catch (e) {}
  }

  function paintCapHint() {
    const el = overlayEl?.querySelector('[data-mehfil-cap-hint]');
    if (!el) return;
    if (!isGroupMehfil() || lastKnownFreshCount < 4) {
      el.hidden = true;
      return;
    }
    const pubs = countLocalPublishers();
    el.hidden = false;
    el.textContent = tt('mehfil_cap_hint', '{{n}}/{{max}} speakers', {
      n: String(pubs),
      max: String(MAX_PUBLISHERS),
    });
    el.title = tt(
      'mehfil_cap_detail',
      'Up to {{pub}} speakers and {{room}} in the room. Extra joiners listen until the host promotes them.',
      { pub: String(MAX_PUBLISHERS), room: String(MAX_PARTICIPANTS) }
    );
  }

  function setPinnedUid(uid) {
    const id = uid ? String(uid) : null;
    if (pinnedUid === id) pinnedUid = null;
    else pinnedUid = id;
    overlayEl?.querySelectorAll('.mehfil-tile[data-uid]').forEach((tile) => {
      tile.classList.toggle('is-pinned', pinnedUid && tile.dataset.uid === pinnedUid);
    });
    scheduleVideoReconcile();
    if (typeof showToast === 'function') {
      showToast(
        pinnedUid
          ? tt('mehfil_pinned', 'Pinned {{name}}', { name: displayNameForUid(pinnedUid) })
          : tt('mehfil_unpinned', 'Pin cleared')
      );
    }
  }

  function queueVolumePaint(volumes) {
    pendingVolumes = volumes;
    const now = Date.now();
    const wait = Math.max(0, VOLUME_PAINT_MS - (now - lastVolumePaintAt));
    if (volumePaintTimer) return;
    volumePaintTimer = setTimeout(() => {
      volumePaintTimer = null;
      lastVolumePaintAt = Date.now();
      const v = pendingVolumes;
      pendingVolumes = null;
      applyVolumeIndicators(v);
      scheduleVideoReconcile();
    }, wait);
  }
  ...
  async function ensureRoomRoles(chatId) {
    ...
    try {
      ...
      if (!cur?.uid) {
        ...
        if (!mySnap?.val()) {
          // Seed roles map from RTDB for accurate publisher count before write.
          try {
            const all = await rtdbRef(`mehfil/${chatId}/roles`)?.once('value');
            rolesByUid.clear();
            Object.entries(all?.val() || {}).forEach(([uid, meta]) => {
              rolesByUid.set(String(uid), String(meta?.role || 'speaker'));
            });
          } catch (e) {}
          const pubs = countLocalPublishers(currentUser.uid);
          if (pubs >= MAX_PUBLISHERS) {
            await myRoleRef.set({ role: 'listener', at: Date.now(), reason: 'publisher_cap' });
            myRoomRole = 'listener';
            lastJoinedAsListenerToast = true;
          } else {
          ...
          if (mySnap.val()?.reason === 'publisher_cap') lastJoinedAsListenerToast = true;
        }
  ...
  async function hostSetRole(targetUid, role) {
    ...
    if (r === 'speaker' && countLocalPublishers(targetUid) >= MAX_PUBLISHERS) {
      if (typeof showToast === 'function') {
        showToast(tt('mehfil_speakers_full', 'Speaker spots are full — demote someone first'));
      }
      return;
    }
    try {
  ...
  function openParticipantModSheet(uid) {
    ...
    if (isSelf) {
      ...
      actions.push(
        `<button type="button" class="mehfil-ring-pick-row" data-mod-pin>${esc(
          pinnedUid === String(uid) ? tt('mehfil_unpin', 'Unpin video') : tt('mehfil_pin', 'Pin video')
        )}</button>`
      );
    }
    sheet.innerHTML = `
    ...
    sheet.querySelector('[data-mod-pin]')?.addEventListener('click', () => {
      close();
      setPinnedUid(uid);
    });
  }
  ...
  async function ensureMehfilParticipant(chatId) {
    ...
    try {
      if (isGroupMehfil()) {
        const ps = await rtdbRef(`mehfil/${chatId}/participants`)?.once('value');
        const state = mehfilLiveState(ps?.val() || {});
        const me = String(currentUser.uid);
        if (state.count >= MAX_PARTICIPANTS && !(state.freshUids || []).map(String).includes(me)) {
          if (typeof showToast === 'function') {
            showToast(tt('mehfil_room_full', 'This Mehfil is full right now — try again in a bit'));
          }
          setMehfilStatus(tt('mehfil_room_full', 'This Mehfil is full right now — try again in a bit'), 'warn');
          return false;
        }
      }
  ...
  function bindMembersList(chatId) {
    ...
    const paintPresenceUi = (snap) => {
      ...
      const freshEntries = (state.freshUids || [])
        .map((uid) => [uid, val[uid]])
        .filter(([, meta]) => meta);
      const freshUidSet = new Set(freshEntries.map(([uid]) => String(uid)));
      if (lastKnownFreshUids.size > 0) {
        freshEntries.forEach(([uid, meta]) => {
          const id = String(uid);
          if (!lastKnownFreshUids.has(id)) {
            queuePresenceEvent('join', { uid: id, name: meta?.name || 'Someone' });
          }
        });
        lastKnownFreshUids.forEach((uid) => {
          if (!freshUidSet.has(uid) && uid !== String(currentUser?.uid || '')) {
            queuePresenceEvent('leave', {
              uid,
              name: lastKnownFreshNames.get(uid) || displayNameForUid(uid) || 'Someone',
            });
          }
        });
      }
      lastKnownFreshCount = state.count;
      lastKnownFreshUids = freshUidSet;
      lastKnownFreshNames = new Map(
        freshEntries.map(([uid, meta]) => [String(uid), meta?.name || 'Someone'])
      );
    ...
    const onPresence = (snap) => {
      // Counts stay accurate immediately; chip DOM is debounced (M7).
      const val = snap.val() || {};
      const state = mehfilLiveState(val);
      lastKnownFreshCount = state.count;
      updateWhoHereStrip(state);
      paintCapHint();
      presencePaintPending = snap;
      clearTimeout(presencePaintTimer);
      presencePaintTimer = setTimeout(() => {
        presencePaintTimer = null;
        if (presencePaintPending) paintPresenceUi(presencePaintPending);
        presencePaintPending = null;
      }, PRESENCE_PAINT_MS);
    };
    ref.on('value', onPresence);
    rtdbUnsubs.push(() => {
      ref.off('value', onPresence);
      clearTimeout(presencePaintTimer);
      presencePaintTimer = null;
    });
  }
  ...
  function updateAloneHint(count) {
    ...
    if (alone) {
      ...
      if (copy) {
        copy.textContent = tt(
          'mehfil_alone_warm',
          'Just you for now — invite someone, or start a watch / listen.'
        );
      }
    }
  }
  ...
  function applyVolumeIndicators(volumes) {
    ...
    overlayEl.querySelectorAll('.mehfil-tile').forEach((tile) => {
      ...
      const speakAnim = speaking && !reduced;
      const speakStatic = speaking && reduced;
      if (tile.classList.contains('is-speaking') !== speakAnim) {
        tile.classList.toggle('is-speaking', speakAnim);
      }
      if (tile.classList.contains('is-speaking-static') !== speakStatic) {
        tile.classList.toggle('is-speaking-static', speakStatic);
      }
  ...
  async function publishCamera() {
    ...
    if (agoraVoiceRole === 'subscriber' || myRoomRole === 'listener') return;
    let remoteCams = 0;
    overlayEl?.querySelectorAll('.mehfil-tile:not(.mehfil-tile--self) video').forEach(() => {
      remoteCams += 1;
    });
    if (remoteCams >= MAX_CAM_ON) {
      if (typeof showToast === 'function') {
        showToast(tt('mehfil_cam_cap', 'Too many cameras on — pin someone or try later'));
      }
      return;
    }
  ...
  function bindMediaSync() {
    ...
    const handler = (snap) => {
      ...
      if (!m || !m.type) {
        if (lastMediaMomentKey) {
          lastMediaMomentKey = '';
          appendSystemMoment(tt('mehfil_moment_media_stopped', 'Media stopped'));
        }
        stopLocalMediaPlayers();
        overlayEl?.classList.remove('mehfil-has-youtube', 'mehfil-has-music');
        layoutCinema();
      ...
      const momentKey = `${m.type || ''}:${m.id || m.url || m.title || ''}`;
      if (momentKey && momentKey !== lastMediaMomentKey) {
        lastMediaMomentKey = momentKey;
        if (m.type === 'youtube') {
          appendSystemMoment(
            tt('mehfil_moment_watch', 'Someone started a watch{{title}}', {
              title: m.title ? ` · ${m.title}` : '',
            })
          );
        } else if (m.type === 'music') {
          appendSystemMoment(
            tt('mehfil_moment_music', 'Music playing{{title}}', {
              title: m.title ? ` · ${m.title}` : '',
            })
          );
        }
    ...
    if (reactRef) {
      const onReact = (snap) => {
        const key = snap.key;
        if (key && reactSeenKeys.has(key)) return;
        if (key) {
          reactSeenKeys.add(key);
          if (reactSeenKeys.size > 80) reactSeenKeys = new Set([...reactSeenKeys].slice(-50));
        }
        const v = snap.val();
        if (!v?.emoji) return;
        // Own sends already burst locally
        if (v.by && v.by === currentUser?.uid && Date.now() - (Number(v.at) || 0) < 2500) return;
        const label = (v.name || displayNameForUid(v.by) || '').split(' ')[0] || '';
        if (v.sticker) showStickerBurst(v.emoji, { label });
        else showReactionBurst(v.emoji, { label });
      };
      reactRef.limitToLast(8).on('child_added', onReact);
  ...
  async function teardownMehfil(reason) {
    ...
    videoPausedNoticeAt = 0;
    speakingSticky.clear();
    mutedByUid.clear();
    lastSpeakingLabel = '';
    chatUnread = 0;
    chatSeenKeys = new Set();
    reactSeenKeys = new Set();
    lastMediaMomentKey = '';
    presenceJoinBuffer = [];
    presenceLeaveBuffer = [];
    clearTimeout(presenceCoalesceTimer);
    ...
    reactSendTimestamps = [];
    try {
      if (typeof MehfilEffects?.clear === 'function') MehfilEffects.clear();
    } catch (e) {}
    clearRtdb();
    try {
      if (localAudio) {
  ...
  async function nudgeOthers() {
    ...
    const text = tt('mehfil_nudge_text', '{{name}} invited you to join the Mehfil', { name });
    try {
      if (typeof sendRealtimeMessage === 'function') {
        await sendRealtimeMessage(activeChatId, text, activeChat.type === 'group', null, {
          type: 'mehfil_invite',
          label: tt('mehfil_join_cta', 'Join the Mehfil'),
        });
      }
  ...
  function ensureRemoteTile(uid, label) {
    ...
    if (tile) {
      ...
      tile.classList.toggle('is-pinned', pinnedUid && String(uid) === pinnedUid);
      return tile;
    }
    tile = document.createElement('div');
    tile.className = 'mehfil-tile mehfil-tile--voice' + (parent === rail ? ' mehfil-tile--rail' : '');
    tile.dataset.uid = String(uid);
    if (pinnedUid && String(uid) === pinnedUid) tile.classList.add('is-pinned');
    ...
    tile.addEventListener('click', (e) => {
      if (e.target.closest('.mehfil-tile-video-paused')) {
        setPinnedUid(uid);
        return;
      }
      openParticipantModSheet(uid);
    });
  ...
  async function joinAgora(chatId, gen) {
    ...
    try {
      ...
      client.on('user-joined', (user) => {
        const uid = String(user.uid);
        if (!remoteUsers.has(uid)) remoteUsers.set(uid, { user, hasVideo: false, hasAudio: false });
        else remoteUsers.get(uid).user = user;
      ...
      client.on('user-published', async (user, mediaType) => {
        try {
          const uid = String(user.uid);
          const info = remoteUsers.get(uid) || { user, hasVideo: false, hasAudio: false };
          info.user = user;
          ensureRemoteTile(user.uid, remoteTileLabel(user));
          if (mediaType === 'audio') {
      ...
      client.on('user-unpublished', (user, mediaType) => {
        const uid = String(user.uid);
        const info = remoteUsers.get(uid);
        if (mediaType === 'video') {
          if (info) info.hasVideo = false;
          videoSubUids.delete(uid);
          ...
          if (!user.hasAudio) {
            ...
            tile.classList.remove('is-video-paused');
            if (!tile.querySelector('.mehfil-tile-avatar')) {
              const name = tile.querySelector('.mehfil-tile-label')?.textContent || '?';
              const av = document.createElement('div');
              av.className = 'mehfil-tile-avatar';
        ...
        if (mediaType === 'audio' && info) info.hasAudio = false;
      });

      client.on('user-left', (user) => {
        const uid = String(user.uid);
        overlayEl?.querySelector(`[data-uid="${user.uid}"]`)?.remove();
        speakingSticky.delete(uid);
        remoteUsers.delete(uid);
        videoSubUids.delete(uid);
      ...
      client.on('volume-indicator', (volumes) => queueVolumePaint(volumes));
      ...
      scheduleTokenRenewal(chatId, tokenPayload.expiresAt);
      agoraVoiceRole =
        tokenPayload.voiceRole === 'subscriber' || tokenPayload.role === 'subscriber'
          ? 'subscriber'
          : 'publisher';
      try {
      ...
      if (lastJoinedAsListenerToast || tokenPayload.voiceReason === 'publisher_cap') {
        ...
        if (typeof showToast === 'function') {
          showToast(
            tt(
              'mehfil_joined_listener',
              'Room is full for speakers — you joined as a listener'
            )
          );
        }
      }
      ...
      if (agoraVoiceRole === 'subscriber' || myRoomRole === 'listener') {
        ...
        renderLocalPlaceholder(tt('mehfil_cam_off', 'Camera off'));
        setMehfilStatus(
          tt('mehfil_listener_mode', 'Listening only — host can let you speak'),
          'warn'
        );
        updateWaitingState();
        paintCapHint();
        const okPresence = await ensureMehfilParticipant(chatId);
        if (!okPresence) {
  ...
  async function openMehfil(chat) {
    ...
    if (!allowed) {
      if (typeof showToast === 'function') {
        showToast(
          tt(
            'mehfil_not_member',
            'You need to be in this chat to join the Mehfil. Ask someone to invite you.'
          )
        );
      }
      return;
    ...
    el.innerHTML = `
      <div class="mehfil-top">
        ...
        <div class="mehfil-glance">
          ...
          <button type="button" class="mehfil-cap-hint" data-mehfil-cap-hint hidden aria-label="${esc(tt('mehfil_cap_detail', 'Speaker capacity'))}"></button>
          ...
          <div class="mehfil-stage" data-mehfil-stage>
            ...
            <div class="mehfil-waiting" data-mehfil-waiting>
              ...
              <div class="mehfil-waiting-actions">
                ...
                <button type="button" class="mehfil-waiting-invite" data-mehfil-waiting-invite>${esc(tt('mehfil_invite_title', 'Invite'))}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="mehfil-alone mehfil-alone-float" data-mehfil-alone hidden>
        <p data-mehfil-alone-copy-text>${esc(tt('mehfil_alone_warm', 'Just you for now — invite someone, or start a watch / listen.'))}</p>
        <div class="mehfil-alone-actions">
          <button type="button" class="mehfil-nudge-btn" data-mehfil-alone-invite>${esc(tt('mehfil_invite_title', 'Invite to Mehfil'))}</button>
          <button type="button" class="mehfil-alone-media" data-mehfil-alone-media>${esc(tt('mehfil_start_something', 'Start something'))}</button>
        </div>
      </div>
      <div class="mehfil-chat-strip is-collapsed" data-mehfil-chat-strip aria-label="${esc(tt('mehfil_chat_expand', 'Room chat'))}">
        ...
        <div class="mehfil-chat-expanded">
          <div class="mehfil-chat-retain" data-mehfil-chat-retain>${esc(tt('mehfil_chat_retain', 'Room chat · live only · not saved to the thread · clears when the room empties'))}</div>
    ...
    applyMehfilTheme(el);
    syncImmersiveChrome();
    el.querySelector('[data-mehfil-cap-hint]')?.addEventListener('click', () => {
      if (typeof showToast === 'function') {
        showToast(
          tt(
            'mehfil_cap_detail',
    ...
    el.querySelector('[data-mehfil-react-quick]')?.addEventListener('click', () => sendRoomReaction('👏', false));
    ...
    el.querySelector('[data-mehfil-chat-form]')?.addEventListener('submit', async (e) => {
      ...
      if (!text) return;
      const ok = await sendRoomChat(text);
      if (ok && input) input.value = '';
      expandChatStrip();
    });
    el.querySelector('[data-mehfil-chat-input]')?.addEventListener('focus', () => {
      expandChatStrip();
      // Keep strip above keyboard inside .device
    ...
    el.querySelector('[data-mehfil-alone-media]')?.addEventListener('click', () => {
      toggleCallSheet('media');
      loadMehfilYtRecs(el);
      setMediaSearchMode(mediaSearchMode);
    });
    ...
    el.querySelector('[data-mehfil-invite-sheet]')?.addEventListener('click', () => {
      closeCallSheets(null);
      openInviteSheet();
    });
    el.querySelectorAll('[data-mehfil-reacts] button, [data-mehfil-stickers] button').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        await sendRoomReaction(emoji, isSticker);
        pokeChrome();
      });
    });
    ...
    // Defaults: groups stay collapsed; DMs open chat strip (call-like, few people).
    if (!isGroup) {
      expandChatStrip();
    }
